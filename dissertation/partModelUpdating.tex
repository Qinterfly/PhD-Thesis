...
Ранее коллективом авторов, включая автора настоящей диссертации, был описан способ коррекции частот для редуцированных КЭ-моделей~\cite{lib:author:spacecraft:cms}. Обобщенные массы и жесткости корректируемого тона изменялись пропорционально коэффициентам $ (1 - k) $ и $ (1 + k) $ соответственно, где $ k $ определялось из равенства отношения обобщенных характеристик квадрату экспериментальной частоты $ \omega_i $. Таким образом, для коррекции $ i $-го тона использовались следующие формулы:
\begin{equation}
