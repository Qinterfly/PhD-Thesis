...
\begin{equation}
	\begin{gathered}
		\partDer{h_1}{\eta_j}{} = y_j \rbrackets{\sum \limits_{i\,=\,1} ^ n h_{i, j} y_i + \sum \limits_{i\,=\,1,\ i \neq j} ^ n h_{j, i} y_i}, \\
		\partDer{h_2}{\eta_j}{} = y_j \rbrackets{\sum \limits_{i\,=\,1} ^ n h_{i, j} \eta_i y_i + \sum \limits_{i\,=\,1,\ i \neq j} ^ n h_{j, i} \eta_i y_i}.
	\end{gathered}
...
\begin{equation}
	\begin{gathered}
		h_1 = \sum \limits_{j\,=\,1} ^ n \sbrackets{\eta_j y_j \rbrackets{2 \sum \limits_{i\,=\,1} ^ n h_{i,j} y_i - h_{j, j} y_j}} \\
		h_2 = \sum \limits_{j\,=\,1} ^ n  \sbrackets{\eta_j y_j \sum \limits_{i\,=\,1, i \neq j} ^ n h_{i, j} \eta_i y_i}, \partDer{h_2}{\eta_j}{} = 2 y_j \sum \limits_{i\,=\,1, i \neq j} ^ n h_{i, j} \eta_i y_i.
	\end{gathered}
\end{equation}
...
\begin{equation}
	\begin{gathered}
		\partDer{h_1}{\eta_j}{} = y_j \rbrackets{2 \sum \limits_{i\,=\,1} ^ n h_{i, j} y_i - h_{j, j} y_j}, \\
		\partDer{h_2}{\eta_j}{} = 2 y_j \rbrackets{\sum \limits_{i\,=\,1} ^ n h_{i, j} \eta_i y_i - h_{j, j} \eta_j y_j}.
	\end{gathered}
\end{equation}

Ранее коллективом авторов, включая автора настоящей диссертации, был описан способ коррекции частот для редуцированных КЭ-моделей \cite{lib:reducedUpdating}. Следуя описанной в работе методологии, матрица демпфирования редуцированной модели может быть восстановлена следующим образом:
\begin{equation}
	\mat{H} ^ \ast = 
